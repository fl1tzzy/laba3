...
// Объявление функции test_miller, которая будет использоваться в дальнейшем
...
// Функция для построения чисел с использованием теста Миллера
...
// Функция для проверки числа на простоту с использованием теста Миллера
tuple<string, int> test_miller(int n, vector<int>& c, int t) {
    int n1 = n - 1;
    vector<int> delit;
    for (int i = 0; i < c.size(); i++) {
...
int main() {
    ...
    c = resheto(500); // Получение вектора простых чисел до 500
    int p;
